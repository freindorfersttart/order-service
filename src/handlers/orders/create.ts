...
const beneficiaryInlineSchema = z.object({
  name: z.string().min(2),
  document: z.string().min(11), // cpf/cnpj com ou sem máscara
...
export const create: APIGatewayProxyHandler = async (event) => {
  try {
    ...
    // ✅ valida conta pagadora existe e está ativa e é PAYOUT
    ...
    if (!payer) {
      return { statusCode: 404, body: JSON.stringify({ error: "bank_account_id não encontrado" }) };
    }
    if (!payer.active) {
      return { statusCode: 400, body: JSON.stringify({ error: "Conta pagadora está inativa" }) };
    }
    ...
    // 🚫 proíbe amount no destino agora
    if (d0.amount != null) {
      return {
        statusCode: 400,
        body: JSON.stringify({
    ...
    // valida destinos: PIX usa destination_pix_key, outros usam destination
    const hasPix = !!d0.destination_pix_key || !!d0.beneficiary?.pix_key; // ✅ permite vir pelo inline
    const hasGeneric = !!d0.destination;

    if (hasPix && hasGeneric) {
      return {
        statusCode: 400,
        body: JSON.stringify({ error: "Destino inválido: use destination_pix_key OU destination (não ambos)." }),
      };
    }
    ...
    if (!isPix && !hasGeneric) {
      return { statusCode: 400, body: JSON.stringify({ error: `${data.type} exige destination (genérico).` }) };
    }

    // ✅ PIX exige beneficiário (id ou inline), pq OnlyUp exige documento
    ...
    // idempotência: busca por id ou idempotency_key
    ...
    const created = await prisma.$transaction(async (tx) => {
      // 1) resolve/garante beneficiary (snapshot)
      ...
      // 1b) carrega beneficiary (se tiver id) e valida ownership
      ...
      if (isPix && !destinationPixKey) {
        throw new Error("Não foi possível resolver destination_pix_key para o beneficiário.");
      }
      ...
      // 5) cria N subtransações com snapshot e idempotency UUID
